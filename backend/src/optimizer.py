...
def buildgeneratetestsprompt(
    ...
    instructions = (
        ...
        "- The code you are testing will be saved as `code.py` in the test environment.\n"
        "- You MUST import it using `import code` or `from code import ...` regardless of the original filename.\n"
        "- Example: `from code import MyClass`\n\n"
        "RESPONSE RULES:\n"
        "- Return ONLY the generated test code.\n"
        "- Place the test code within appropriate file blocks (e.g., `**File: test_code.py**`).\n"
